...
BASELIB_STATIC_ASSERT(sizeof(Baselib_BinarySemaphore) == PLATFORM_CACHE_LINE_SIZE * 2, "Baselib_BinarySemaphore (Futex) size should match 2 * cacheline size");
BASELIB_STATIC_ASSERT(offsetof(Baselib_BinarySemaphore, wakeups) == (offsetof(Baselib_BinarySemaphore, state) - PLATFORM_CACHE_LINE_SIZE), "Baselib_BinarySemaphore (futex) wakeups and count shouldn't share cacheline");
