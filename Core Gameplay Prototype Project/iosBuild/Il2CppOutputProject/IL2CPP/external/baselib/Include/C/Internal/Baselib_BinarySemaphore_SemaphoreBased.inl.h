...
typedef struct Baselib_BinarySemaphore
{
    ...
    char _cachelineSpacer[PLATFORM_CACHE_LINE_SIZE - sizeof(int32_t) - sizeof(Baselib_SystemSemaphore_Handle)];
    char _systemSemaphoreData[Baselib_SystemSemaphore_PlatformSize];
} Baselib_BinarySemaphore;

BASELIB_STATIC_ASSERT((offsetof(Baselib_BinarySemaphore, state) + PLATFORM_CACHE_LINE_SIZE - sizeof(Baselib_SystemSemaphore_Handle)) ==
